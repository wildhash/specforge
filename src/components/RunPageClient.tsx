/**
 * SpecForge — Run Client Component
 * Client-side logic for run status page
 */
...
export function RunPageClient() {
  ...
  useEffect(() => {
    ...
    if (DEMO_MODE) {
      // Demo mode: simulate pipeline progress
      let progress = 0;
      const demoIdea = typeof window !== 'undefined' 
        ? localStorage.getItem('demoIdea') || 'A revolutionary AI product builder'
        : 'A revolutionary AI product builder';
      
      const simulateProgress = () => {
        if (stopped) return;
        
        progress += 5;
        const mockState = simulatePipelineProgress(demoIdea, runId as string, progress);
        
        setState(mockState);
        setComplete(progress >= 100);
        setPipelineStatus(mockState.orchestrationPlan.pipelineStatus);
        setLoadError(null);
      ...
      simulateProgress();
      intervalId = setInterval(simulateProgress, 800);
